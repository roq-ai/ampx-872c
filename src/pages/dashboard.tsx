...
import { ChargingSessionListPage as ShowCaseList } from 'pages/charging-sessions';

function DashboardPage() {
  const { session } = useSession();

  const chargingRequestStat = useSWR<PaginatedInterface<ChargingRequestInterface>>(
    () => `/charging-requests`,
    () => getChargingRequests({ limit: 1, offset: 0 }),
  );

  const serviceVehicleStat = useSWR<PaginatedInterface<ServiceVehicleInterface>>(
    () => `/service-vehicles`,
    () => getServiceVehicles({ limit: 1, offset: 0 }),
  );

  const carOwnerStat = useSWR<PaginatedInterface<CarOwnerInterface>>(
    () => `/car-owners`,
    () => getCarOwners({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Charging Requests',
      result: chargingRequestStat?.data?.totalCount,
      isLoading: chargingRequestStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Service Vehicles',
      result: serviceVehicleStat?.data?.totalCount,
      isLoading: serviceVehicleStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Car Owners',
      result: carOwnerStat?.data?.totalCount,
      isLoading: carOwnerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`charging-sessions`} passHref legacyBehavior>
                  <Button
