...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getCarOwners();
    case 'POST':
      return createCarOwner();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getCarOwners() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const response = await prisma.car_owner
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'car_owner'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createCarOwner() {
    ...
    const data = await prisma.car_owner.create({
      data: body,
    });
