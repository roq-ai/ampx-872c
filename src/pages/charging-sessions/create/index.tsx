...
import { createChargingSession } from 'apiSdk/charging-sessions';
...
function ChargingSessionCreatePage() {
  ...
  const handleSubmit = async (values: ChargingSessionInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createChargingSession(values);
      resetForm();
  ...
  const formik = useFormik<ChargingSessionInterface>({
    initialValues: {
      ...
      charge_provided: 0,
      session_status: '',
      charging_request_id: (router.query.charging_request_id as string) ?? null,
